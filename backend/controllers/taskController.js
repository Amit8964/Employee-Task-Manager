...
const setTask = async (req, res) => {
    ...
    try {
        ...
        data.save().then(async (resp) => {
            ...
            let data = await employee.findOneAndUpdate(resp.assign, { $push: { tasks: resp._id } })
            console.log(resp);
...
const deleteTask = async (req, res) => {
    ...
    try {
        ...
        console.log(data)
        if (data.assign != null) {
            let employeeData = await employee.findOneAndUpdate({ _id: data.assign }, { $pull: { tasks: data._id } },
                { new: true });
